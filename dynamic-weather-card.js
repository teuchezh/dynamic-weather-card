var e=function(k,F,K,Z){var _=arguments.length,N=_<3?F:Z===null?Z=Object.getOwnPropertyDescriptor(F,K):Z,W;if(typeof Reflect==="object"&&typeof Reflect.decorate==="function")N=Reflect.decorate(k,F,K,Z);else for(var q=k.length-1;q>=0;q--)if(W=k[q])N=(_<3?W(N):_>3?W(F,K,N):W(F,K))||N;return _>3&&N&&Object.defineProperty(F,K,N),N};var t=globalThis,$0=t.ShadowRoot&&(t.ShadyCSS===void 0||t.ShadyCSS.nativeShadow)&&"adoptedStyleSheets"in Document.prototype&&"replace"in CSSStyleSheet.prototype,B0=Symbol(),O0=new WeakMap;class J0{constructor(k,F,K){if(this._$cssResult$=!0,K!==B0)throw Error("CSSResult is not constructable. Use `unsafeCSS` or `css` instead.");this.cssText=k,this._strings=F}get styleSheet(){let k=this._styleSheet,F=this._strings;if($0&&k===void 0){let K=F!==void 0&&F.length===1;if(K)k=O0.get(F);if(k===void 0){if((this._styleSheet=k=new CSSStyleSheet).replaceSync(this.cssText),K)O0.set(F,k)}}return k}toString(){return this.cssText}}var P1=(k)=>{if(k._$cssResult$===!0)return k.cssText;else if(typeof k==="number")return k;else throw Error(`Value passed to 'css' function must be a 'css' function result: ${k}. Use 'unsafeCSS' to pass non-literal values, but take care to ensure page security.`)},D1=(k)=>new J0(typeof k==="string"?k:String(k),void 0,B0),l0=(k,...F)=>{let K=k.length===1?k[0]:F.reduce((Z,_,N)=>Z+P1(_)+k[N+1],k[0]);return new J0(K,k,B0)},d0=(k,F)=>{if($0)k.adoptedStyleSheets=F.map((K)=>K instanceof CSSStyleSheet?K:K.styleSheet);else for(let K of F){let Z=document.createElement("style"),_=t.litNonce;if(_!==void 0)Z.setAttribute("nonce",_);Z.textContent=K.cssText,k.appendChild(Z)}},c1=(k)=>{let F="";for(let K of k.cssRules)F+=K.cssText;return D1(F)},Q0=$0?(k)=>k:(k)=>k instanceof CSSStyleSheet?c1(k):k;var{is:O1,defineProperty:l1,getOwnPropertyDescriptor:E0,getOwnPropertyNames:d1,getOwnPropertySymbols:E1,getPrototypeOf:u0}=Object,u1=!1,U=globalThis;if(u1)U.customElements??=customElements;var j=!0,X,h0=U.trustedTypes,h1=h0?h0.emptyScript:"",v0=j?U.reactiveElementPolyfillSupportDevMode:U.reactiveElementPolyfillSupport;if(j)U.litIssuedWarnings??=new Set,X=(k,F)=>{if(F+=` See https://lit.dev/msg/${k} for more information.`,!U.litIssuedWarnings.has(F)&&!U.litIssuedWarnings.has(k))console.warn(F),U.litIssuedWarnings.add(F)},queueMicrotask(()=>{if(X("dev-mode","Lit is in dev mode. Not recommended for production!"),U.ShadyDOM?.inUse&&v0===void 0)X("polyfill-support-missing","Shadow DOM is being polyfilled via `ShadyDOM` but the `polyfill-support` module has not been loaded.")});var o1=j?(k)=>{if(!U.emitLitDebugLogEvents)return;U.dispatchEvent(new CustomEvent("lit-debug",{detail:k}))}:void 0,O=(k,F)=>k,d={toAttribute(k,F){switch(F){case Boolean:k=k?h1:null;break;case Object:case Array:k=k==null?k:JSON.stringify(k);break}return k},fromAttribute(k,F){let K=k;switch(F){case Boolean:K=k!==null;break;case Number:K=k===null?null:Number(k);break;case Object:case Array:try{K=JSON.parse(k)}catch(Z){K=null}break}return K}},k0=(k,F)=>!O1(k,F),o0={attribute:!0,type:String,converter:d,reflect:!1,useDefault:!1,hasChanged:k0};Symbol.metadata??=Symbol("metadata");U.litPropertyMetadata??=new WeakMap;class L extends HTMLElement{static addInitializer(k){this.__prepare(),(this._initializers??=[]).push(k)}static get observedAttributes(){return this.finalize(),this.__attributeToPropertyMap&&[...this.__attributeToPropertyMap.keys()]}static createProperty(k,F=o0){if(F.state)F.attribute=!1;if(this.__prepare(),this.prototype.hasOwnProperty(k))F=Object.create(F),F.wrapped=!0;if(this.elementProperties.set(k,F),!F.noAccessor){let K=j?Symbol.for(`${String(k)} (@property() cache)`):Symbol(),Z=this.getPropertyDescriptor(k,K,F);if(Z!==void 0)l1(this.prototype,k,Z)}}static getPropertyDescriptor(k,F,K){let{get:Z,set:_}=E0(this.prototype,k)??{get(){return this[F]},set(N){this[F]=N}};if(j&&Z==null){if("value"in(E0(this.prototype,k)??{}))throw Error(`Field ${JSON.stringify(String(k))} on ${this.name} was declared as a reactive property but it's actually declared as a value on the prototype. Usually this is due to using @property or @state on a method.`);X("reactive-property-without-getter",`Field ${JSON.stringify(String(k))} on ${this.name} was declared as a reactive property but it does not have a getter. This will be an error in a future version of Lit.`)}return{get:Z,set(N){let W=Z?.call(this);_?.call(this,N),this.requestUpdate(k,W,K)},configurable:!0,enumerable:!0}}static getPropertyOptions(k){return this.elementProperties.get(k)??o0}static __prepare(){if(this.hasOwnProperty(O("elementProperties",this)))return;let k=u0(this);if(k.finalize(),k._initializers!==void 0)this._initializers=[...k._initializers];this.elementProperties=new Map(k.elementProperties)}static finalize(){if(this.hasOwnProperty(O("finalized",this)))return;if(this.finalized=!0,this.__prepare(),this.hasOwnProperty(O("properties",this))){let F=this.properties,K=[...d1(F),...E1(F)];for(let Z of K)this.createProperty(Z,F[Z])}let k=this[Symbol.metadata];if(k!==null){let F=litPropertyMetadata.get(k);if(F!==void 0)for(let[K,Z]of F)this.elementProperties.set(K,Z)}this.__attributeToPropertyMap=new Map;for(let[F,K]of this.elementProperties){let Z=this.__attributeNameForProperty(F,K);if(Z!==void 0)this.__attributeToPropertyMap.set(Z,F)}if(this.elementStyles=this.finalizeStyles(this.styles),j){if(this.hasOwnProperty("createProperty"))X("no-override-create-property","Overriding ReactiveElement.createProperty() is deprecated. The override will not be called with standard decorators");if(this.hasOwnProperty("getPropertyDescriptor"))X("no-override-get-property-descriptor","Overriding ReactiveElement.getPropertyDescriptor() is deprecated. The override will not be called with standard decorators")}}static finalizeStyles(k){let F=[];if(Array.isArray(k)){let K=new Set(k.flat(1/0).reverse());for(let Z of K)F.unshift(Q0(Z))}else if(k!==void 0)F.push(Q0(k));return F}static __attributeNameForProperty(k,F){let K=F.attribute;return K===!1?void 0:typeof K==="string"?K:typeof k==="string"?k.toLowerCase():void 0}constructor(){super();this.__instanceProperties=void 0,this.isUpdatePending=!1,this.hasUpdated=!1,this.__reflectingProperty=null,this.__initialize()}__initialize(){this.__updatePromise=new Promise((k)=>this.enableUpdating=k),this._$changedProperties=new Map,this.__saveInstanceProperties(),this.requestUpdate(),this.constructor._initializers?.forEach((k)=>k(this))}addController(k){if((this.__controllers??=new Set).add(k),this.renderRoot!==void 0&&this.isConnected)k.hostConnected?.()}removeController(k){this.__controllers?.delete(k)}__saveInstanceProperties(){let k=new Map,F=this.constructor.elementProperties;for(let K of F.keys())if(this.hasOwnProperty(K))k.set(K,this[K]),delete this[K];if(k.size>0)this.__instanceProperties=k}createRenderRoot(){let k=this.shadowRoot??this.attachShadow(this.constructor.shadowRootOptions);return d0(k,this.constructor.elementStyles),k}connectedCallback(){this.renderRoot??=this.createRenderRoot(),this.enableUpdating(!0),this.__controllers?.forEach((k)=>k.hostConnected?.())}enableUpdating(k){}disconnectedCallback(){this.__controllers?.forEach((k)=>k.hostDisconnected?.())}attributeChangedCallback(k,F,K){this._$attributeToProperty(k,K)}__propertyToAttribute(k,F){let Z=this.constructor.elementProperties.get(k),_=this.constructor.__attributeNameForProperty(k,Z);if(_!==void 0&&Z.reflect===!0){let W=(Z.converter?.toAttribute!==void 0?Z.converter:d).toAttribute(F,Z.type);if(j&&this.constructor.enabledWarnings.includes("migration")&&W===void 0)X("undefined-attribute-value",`The attribute value for the ${k} property is undefined on element ${this.localName}. The attribute will be removed, but in the previous version of \`ReactiveElement\`, the attribute would not have changed.`);if(this.__reflectingProperty=k,W==null)this.removeAttribute(_);else this.setAttribute(_,W);this.__reflectingProperty=null}}_$attributeToProperty(k,F){let K=this.constructor,Z=K.__attributeToPropertyMap.get(k);if(Z!==void 0&&this.__reflectingProperty!==Z){let _=K.getPropertyOptions(Z),N=typeof _.converter==="function"?{fromAttribute:_.converter}:_.converter?.fromAttribute!==void 0?_.converter:d;this.__reflectingProperty=Z;let W=N.fromAttribute(F,_.type);this[Z]=W??this.__defaultValues?.get(Z)??W,this.__reflectingProperty=null}}requestUpdate(k,F,K,Z=!1,_){if(k!==void 0){if(j&&k instanceof Event)X("","The requestUpdate() method was called with an Event as the property name. This is probably a mistake caused by binding this.requestUpdate as an event listener. Instead bind a function that will call it with no arguments: () => this.requestUpdate()");let N=this.constructor;if(Z===!1)_=this[k];if(K??=N.getPropertyOptions(k),(K.hasChanged??k0)(_,F)||K.useDefault&&K.reflect&&_===this.__defaultValues?.get(k)&&!this.hasAttribute(N.__attributeNameForProperty(k,K)))this._$changeProperty(k,F,K);else return}if(this.isUpdatePending===!1)this.__updatePromise=this.__enqueueUpdate()}_$changeProperty(k,F,{useDefault:K,reflect:Z,wrapped:_},N){if(K&&!(this.__defaultValues??=new Map).has(k)){if(this.__defaultValues.set(k,N??F??this[k]),_!==!0||N!==void 0)return}if(!this._$changedProperties.has(k)){if(!this.hasUpdated&&!K)F=void 0;this._$changedProperties.set(k,F)}if(Z===!0&&this.__reflectingProperty!==k)(this.__reflectingProperties??=new Set).add(k)}async __enqueueUpdate(){this.isUpdatePending=!0;try{await this.__updatePromise}catch(F){Promise.reject(F)}let k=this.scheduleUpdate();if(k!=null)await k;return!this.isUpdatePending}scheduleUpdate(){let k=this.performUpdate();if(j&&this.constructor.enabledWarnings.includes("async-perform-update")&&typeof k?.then==="function")X("async-perform-update",`Element ${this.localName} returned a Promise from performUpdate(). This behavior is deprecated and will be removed in a future version of ReactiveElement.`);return k}performUpdate(){if(!this.isUpdatePending)return;if(o1?.({kind:"update"}),!this.hasUpdated){if(this.renderRoot??=this.createRenderRoot(),j){let _=[...this.constructor.elementProperties.keys()].filter((N)=>this.hasOwnProperty(N)&&(N in u0(this)));if(_.length)throw Error(`The following properties on element ${this.localName} will not trigger updates as expected because they are set using class fields: ${_.join(", ")}. Native class fields and some compiled output will overwrite accessors used for detecting changes. See https://lit.dev/msg/class-field-shadowing for more information.`)}if(this.__instanceProperties){for(let[Z,_]of this.__instanceProperties)this[Z]=_;this.__instanceProperties=void 0}let K=this.constructor.elementProperties;if(K.size>0)for(let[Z,_]of K){let{wrapped:N}=_,W=this[Z];if(N===!0&&!this._$changedProperties.has(Z)&&W!==void 0)this._$changeProperty(Z,void 0,_,W)}}let k=!1,F=this._$changedProperties;try{if(k=this.shouldUpdate(F),k)this.willUpdate(F),this.__controllers?.forEach((K)=>K.hostUpdate?.()),this.update(F);else this.__markUpdated()}catch(K){throw k=!1,this.__markUpdated(),K}if(k)this._$didUpdate(F)}willUpdate(k){}_$didUpdate(k){if(this.__controllers?.forEach((F)=>F.hostUpdated?.()),!this.hasUpdated)this.hasUpdated=!0,this.firstUpdated(k);if(this.updated(k),j&&this.isUpdatePending&&this.constructor.enabledWarnings.includes("change-in-update"))X("change-in-update",`Element ${this.localName} scheduled an update (generally because a property was set) after an update completed, causing a new update to be scheduled. This is inefficient and should be avoided unless the next update can only be scheduled as a side effect of the previous update.`)}__markUpdated(){this._$changedProperties=new Map,this.isUpdatePending=!1}get updateComplete(){return this.getUpdateComplete()}getUpdateComplete(){return this.__updatePromise}shouldUpdate(k){return!0}update(k){this.__reflectingProperties&&=this.__reflectingProperties.forEach((F)=>this.__propertyToAttribute(F,this[F])),this.__markUpdated()}updated(k){}firstUpdated(k){}}L.elementStyles=[];L.shadowRootOptions={mode:"open"};L[O("elementProperties",L)]=new Map;L[O("finalized",L)]=new Map;v0?.({ReactiveElement:L});if(j){L.enabledWarnings=["change-in-update","async-perform-update"];let k=function(F){if(!F.hasOwnProperty(O("enabledWarnings",F)))F.enabledWarnings=F.enabledWarnings.slice()};L.enableWarning=function(F){if(k(this),!this.enabledWarnings.includes(F))this.enabledWarnings.push(F)},L.disableWarning=function(F){k(this);let K=this.enabledWarnings.indexOf(F);if(K>=0)this.enabledWarnings.splice(K,1)}}(U.reactiveElementVersions??=[]).push("2.1.2");if(j&&U.reactiveElementVersions.length>1)queueMicrotask(()=>{X("multiple-versions","Multiple versions of Lit loaded. Loading multiple versions is not recommended.")});var g=globalThis,J=(k)=>{if(!g.emitLitDebugLogEvents)return;g.dispatchEvent(new CustomEvent("lit-debug",{detail:k}))},v1=0,h;g.litIssuedWarnings??=new Set,h=(k,F)=>{if(F+=k?` See https://lit.dev/msg/${k} for more information.`:"",!g.litIssuedWarnings.has(F)&&!g.litIssuedWarnings.has(k))console.warn(F),g.litIssuedWarnings.add(F)},queueMicrotask(()=>{h("dev-mode","Lit is in dev mode. Not recommended for production!")});var Y=g.ShadyDOM?.inUse&&g.ShadyDOM?.noPatch===!0?g.ShadyDOM.wrap:(k)=>k,F0=g.trustedTypes,p0=F0?F0.createPolicy("lit-html",{createHTML:(k)=>k}):void 0,p1=(k)=>k,N0=(k,F,K)=>p1,m1=(k)=>{if(c!==N0)throw Error("Attempted to overwrite existing lit-html security policy. setSanitizeDOMValueFactory should be called at most once.");c=k},i1=()=>{c=N0},b0=(k,F,K)=>{return c(k,F,K)},e0="$lit$",C=`lit$${Math.random().toFixed(9).slice(2)}$`,t0="?"+C,r1=`<${t0}>`,P=document,o=()=>P.createComment(""),v=(k)=>k===null||typeof k!="object"&&typeof k!="function",A0=Array.isArray,s1=(k)=>A0(k)||typeof k?.[Symbol.iterator]==="function",f0=`[ 	
\f\r]`,n1=`[^ 	
\f\r"'\`<>=]`,a1=`[^\\s"'>=/]`,E=/<(?:(!--|\/[^a-zA-Z])|(\/?[a-zA-Z][^>\s]*)|(\/?$))/g,m0=1,G0=2,e1=3,i0=/-->/g,r0=/>/g,I=new RegExp(`>|${f0}(?:(${a1}+)(${f0}*=${f0}*(?:${n1}|("|')|))|$)`,"g"),t1=0,s0=1,kk=2,n0=3,H0=/'/g,y0=/"/g,k1=/^(?:script|style|textarea|title)$/i,Fk=1,K0=2,Z0=3,M0=1,_0=2,Kk=3,Zk=4,_k=5,U0=6,Nk=7,j0=(k)=>(F,...K)=>{if(F.some((Z)=>Z===void 0))console.warn(`Some template strings are undefined.
...
Please use the static 'html' tag function. See https://lit.dev/docs/templates/expressions/#static-expressions`);return{["_$litType$"]:k,strings:F,values:K}},G=j0(Fk),f=j0(K0),ck=j0(Z0),D=Symbol.for("lit-noChange"),b=Symbol.for("lit-nothing"),a0=new WeakMap,w=P.createTreeWalker(P,129),c=N0;function F1(k,F){if(!A0(k)||!k.hasOwnProperty("raw")){let K="invalid template strings array";throw K=`
...
`,Yk={sunny:f`
  ...
  `};function n(k,...F){let K=Xk[k];if(typeof K==="function")return K(...F);return K||""}function w0(k){if(!k)return"";return Yk[k.toLowerCase()]||""}class P0 extends R{animationFrame=null;canvas=null;ctx=null;canvasWidth=0;canvasHeight=0;animations={};holdTimer=null;holdDelay=500;clockInterval=null;resizeObserver=null;_wheelHandler=null;lastTap=null;holdFired=!1;_testTimeOfDay;static get styles(){return x1}constructor(){super();this.currentTime="";this.config={}}connectedCallback(){super.connectedCallback(),this.updateComplete.then(()=>{setTimeout(()=>{if(this.setupCanvas(),this.canvas&&this.ctx)this.initializeAnimations(),this.startAnimation(),this.setupResizeObserver();this.setupForecastScroll(),this.startClock()},100)})}disconnectedCallback(){if(super.disconnectedCallback(),this.animationFrame)cancelAnimationFrame(this.animationFrame),this.animationFrame=null;if(this.resizeObserver)this.resizeObserver.disconnect(),this.resizeObserver=null;if(this._wheelHandler&&this.shadowRoot){let k=this.shadowRoot.querySelector(".forecast-scroll");if(k)k.removeEventListener("wheel",this._wheelHandler);this._wheelHandler=null}if(this.clockInterval)clearInterval(this.clockInterval),this.clockInterval=null}updated(k){if(super.updated(k),k.has("hass")||k.has("config")){if(this.canvas&&this.ctx)this.resizeCanvas();this.setupForecastScroll()}let F=X0({configLang:this.config?.language,hassLang:this.hass?.language});if(y.lang!==F)y.setLanguage(F)}initializeAnimations(){if(!this.ctx)return;this.animations={sunny:new z0(this.ctx),rainy:new s(this.ctx),snowy:new T0(this.ctx),cloudy:new C0(this.ctx),foggy:new S0(this.ctx),hail:new I0(this.ctx),thunderstorm:new R0(this.ctx)}}setupResizeObserver(){if(!this.shadowRoot)return;let k=this.shadowRoot.querySelector(".canvas-container");if(!k)return;this.resizeObserver=new ResizeObserver(()=>{this.resizeCanvas()}),this.resizeObserver.observe(k)}setupForecastScroll(){if(!this.shadowRoot)return;let k=this.shadowRoot.querySelector(".forecast-scroll");if(!k)return;if(this._wheelHandler)k.removeEventListener("wheel",this._wheelHandler);this._wheelHandler=(F)=>{let K=F;if(K.deltaY!==0)F.preventDefault(),k.scrollLeft+=K.deltaY},k.addEventListener("wheel",this._wheelHandler,{passive:!1})}resizeCanvas(){if(!this.canvas||!this.shadowRoot)return;let k=this.shadowRoot.querySelector(".canvas-container");if(!k)return;let F=k.getBoundingClientRect();if(F.width===0||F.height===0)return;let K=window.devicePixelRatio||2;if(this.canvas.width=F.width*K,this.canvas.height=F.height*K,this.canvas.style.width="100%",this.canvas.style.height="100%",this.ctx=this.canvas.getContext("2d"),this.ctx)this.ctx.scale(K,K);this.canvasWidth=F.width,this.canvasHeight=F.height,this.initializeAnimations()}setupCanvas(){if(!this.shadowRoot)return;let k=this.shadowRoot.querySelector(".canvas-container");if(!k)return;let F=k.querySelector("canvas");if(F)F.remove();this.canvas=document.createElement("canvas"),k.appendChild(this.canvas),this.resizeCanvas()}getState(k){if(!this.hass||!k)return null;let F=this.hass.states[k];return F?F.state:null}getAttributes(k){if(!this.hass||!k)return{};let F=this.hass.states[k];return F?F.attributes:{}}getWeatherData(){let k=this.config.entity||"weather.home",F=this.getState(k),K=this.getAttributes(k),Z=K.condition||F||"sunny",_=null;if(this.config.templowAttribute&&K[this.config.templowAttribute]!=null)_=K[this.config.templowAttribute];else{for(let N of f1)if(K[N]!=null){_=K[N];break}if(_==null)_=(K.forecast&&K.forecast[0]?K.forecast[0].templow??null:null)||(K.forecast_hourly&&K.forecast_hourly[0]?K.forecast_hourly[0].native_templow??null:null)}return{condition:Z,temperature:K.temperature!=null?K.temperature:null,apparentTemperature:K.apparent_temperature||null,humidity:K.humidity!=null?K.humidity:null,windSpeed:K.wind_speed!=null?K.wind_speed:null,windGust:K.wind_gust_speed||K.wind_gust||null,windBearing:K.wind_bearing!=null?K.wind_bearing:null,windDirection:K.wind_direction||null,pressure:K.pressure||null,forecast:K.forecast||K.forecast_hourly||[],friendlyName:K.friendly_name||y.t("weather"),templow:_}}getTodayForecast(){if(!this.hass||!this.config)return[];let k=this.getWeatherData();if(!k.forecast||k.forecast.length===0)return[];let F=new Date,K=new Date(F.getFullYear(),F.getMonth(),F.getDate()),Z=new Date(K);Z.setDate(Z.getDate()+1);let _=k.forecast.filter((W)=>{if(!W.datetime)return!1;let q=new Date(W.datetime),$=new Date(q.getFullYear(),q.getMonth(),q.getDate());return $.getTime()===K.getTime()||$.getTime()===Z.getTime()&&q.getHours()<=F.getHours()}),N=Math.max(1,Math.floor(Number(this.config.hourlyForecastHours??x.hourlyForecastHours)));return _.sort((W,q)=>new Date(W.datetime).getTime()-new Date(q.datetime).getTime()).slice(0,N)}getWeekForecast(){if(!this.hass||!this.config)return[];let k=this.getWeatherData();if(!k.forecast||k.forecast.length===0)return[];let F=Math.max(1,Math.floor(Number(this.config.dailyForecastDays??x.dailyForecastDays))),K=new Date,Z=new Date(K.getFullYear(),K.getMonth(),K.getDate()),_=new Date(Z);_.setDate(_.getDate()+F);let N=(q)=>{let $=q.getFullYear(),Q=String(q.getMonth()+1).padStart(2,"0"),B=String(q.getDate()).padStart(2,"0");return`${$}-${Q}-${B}`},W=new Map;return k.forecast.forEach((q)=>{if(!q.datetime)return;let $=new Date(q.datetime);if(Number.isNaN($.getTime()))return;if($<Z||$>=_)return;let Q=N($),B=Math.abs($.getHours()+$.getMinutes()/60-12),H=W.get(Q);if(!H||B<H.hourScore)W.set(Q,{item:q,itemDate:$,hourScore:B})}),Array.from(W.values()).sort((q,$)=>q.itemDate.getTime()-$.itemDate.getTime()).map((q)=>q.item).slice(0,F)}startAnimation(){let k=()=>{this.draw(),this.animationFrame=requestAnimationFrame(k)};k()}draw(){if(!this.ctx||!this.canvas)return;if(!this.canvasWidth||!this.canvasHeight){if(this.resizeCanvas(),!this.canvasWidth||!this.canvasHeight)return}let k=this.canvasWidth,F=this.canvasHeight;this.ctx.clearRect(0,0,k,F);let K=this.getWeatherData(),Z=this.hass?.states[this.config.entity],_=x0(Z||{},this.config.sunriseEntity,this.config.sunsetEntity,this.hass),N=this._testTimeOfDay||V0(_);switch(K.condition.toLowerCase()){case"sunny":case"clear":this.animations.sunny?.draw(Date.now(),k,F,N);break;case"clear-night":this.animations.sunny?.draw(Date.now(),k,F,{type:"night",progress:0});break;case"rainy":case"rain":this.animations.rainy?.draw(Date.now(),k,F,N,!1);break;case"pouring":this.animations.rainy?.draw(Date.now(),k,F,N,!0);break;case"snowy":case"snow":this.animations.snowy?.draw(Date.now(),k,F,N);break;case"snowy-rainy":this.animations.rainy?.draw(Date.now(),k,F,N,!1),this.animations.snowy?.draw(Date.now(),k,F,N);break;case"hail":this.animations.hail?.draw(Date.now(),k,F,N);break;case"foggy":case"fog":this.animations.foggy?.draw(Date.now(),k,F,N);break;case"lightning":this.animations.thunderstorm?.draw(Date.now(),k,F,N,!1);break;case"lightning-rainy":this.animations.thunderstorm?.draw(Date.now(),k,F,N,!0);break;case"cloudy":case"partlycloudy":default:this.animations.cloudy?.draw(Date.now(),k,F,N);break}}renderTodayForecast(){let k=this.getTodayForecast();if(k.length===0)return G`<div style="opacity: 0.6; font-size: 14px;">${y.t("forecast_unavailable")}</div>`;return G`
    ...
    `}convertWindSpeed(k){if(k==null)return null;if(this.config.windSpeedUnit==="kmh")return Math.round(k*3.6*10)/10;return k}getWindSpeedUnit(){return this.config.windSpeedUnit==="kmh"?y.t("wind_unit_kmh"):y.t("wind_unit_ms")}formatCurrentTime(){let k=new Date,F=String(k.getHours()).padStart(2,"0"),K=String(k.getMinutes()).padStart(2,"0");return`${F}:${K}`}startClock(){if(!this.config.showClock)return;this.currentTime=this.formatCurrentTime(),this.clockInterval=window.setInterval(()=>{this.currentTime=this.formatCurrentTime()},1000)}render(){if(!this.hass)return G`<div>No Home Assistant connection</div>`;let k=this.getWeatherData(),F=this.hass.states[this.config.entity],K=x0(F,this.config.sunriseEntity,this.config.sunsetEntity,this.hass),Z=this._testTimeOfDay||V0(K),_=`weather-card ${Z.type}`,N=this.config.height?`${this.config.height}px`:"200px",W=g1(Z),q=W?`background: linear-gradient(135deg, rgb(${W.start.r}, ${W.start.g}, ${W.start.b}), rgb(${W.end.r}, ${W.end.g}, ${W.end.b}));`:"",Q=`--overlay-opacity: ${this.config.overlayOpacity!==void 0?this.config.overlayOpacity:x.overlayOpacity};`;return G`
      <ha-card
