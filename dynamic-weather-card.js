...
`;function u($,...J){const q=T1[$];if(typeof q==="function")return q(...J);return q||""}function R0($){if(!$)return"";return D1[$.toLowerCase()]||""}var T1={wind:j`
...
`,D1={sunny:j`
  ...
  `};class _0 extends G{animationFrame=null;canvas=null;ctx=null;canvasWidth=0;canvasHeight=0;animations={};holdTimer=null;holdDelay=500;clockInterval=null;resizeObserver=null;_wheelHandler=null;lastTap=null;holdFired=!1;_testTimeOfDay;static get styles(){return J1}static getConfigElement(){return document.createElement("dynamic-weather-card-editor")}static getStubConfig(){return{type:"custom:dynamic-weather-card",entity:"weather.home",show_hourly_forecast:!0,hourly_forecast_hours:P.hourlyForecastHours,show_daily_forecast:!0,daily_forecast_days:P.dailyForecastDays}}constructor(){super();this.currentTime="";this.config={}}connectedCallback(){super.connectedCallback(),this.updateComplete.then(()=>{setTimeout(()=>{if(this.setupCanvas(),this.canvas&&this.ctx)this.initializeAnimations(),this.startAnimation(),this.setupResizeObserver();this.setupForecastScroll(),this.startClock()},100)})}disconnectedCallback(){if(super.disconnectedCallback(),this.animationFrame)cancelAnimationFrame(this.animationFrame),this.animationFrame=null;if(this.resizeObserver)this.resizeObserver.disconnect(),this.resizeObserver=null;if(this._wheelHandler&&this.shadowRoot){const $=this.shadowRoot.querySelector(".forecast-scroll");if($)$.removeEventListener("wheel",this._wheelHandler);this._wheelHandler=null}if(this.clockInterval)clearInterval(this.clockInterval),this.clockInterval=null}updated($){if(super.updated($),$.has("hass")||$.has("config")){if(this.canvas&&this.ctx)this.resizeCanvas();this.setupForecastScroll()}if($.has("config"))this.startClock();const J=Z0({configLang:this.config?.language,hassLang:this.hass?.language});if(U.lang!==J)U.setLanguage(J)}initializeAnimations(){if(!this.ctx)return;this.animations={sunny:new b0(this.ctx),rainy:new l(this.ctx),snowy:new P0(this.ctx),cloudy:new M0(this.ctx),foggy:new j0(this.ctx),hail:new x0(this.ctx),thunderstorm:new U0(this.ctx)}}setupResizeObserver(){if(!this.shadowRoot)return;const $=this.shadowRoot.querySelector(".canvas-container");if(!$)return;this.resizeObserver=new ResizeObserver(()=>{this.resizeCanvas()}),this.resizeObserver.observe($)}setupForecastScroll(){if(!this.shadowRoot)return;const $=this.shadowRoot.querySelector(".forecast-scroll");if(!$)return;if(this._wheelHandler)$.removeEventListener("wheel",this._wheelHandler);this._wheelHandler=(J)=>{const q=J;if(q.deltaY!==0)J.preventDefault(),$.scrollLeft+=q.deltaY},$.addEventListener("wheel",this._wheelHandler,{passive:!1})}resizeCanvas(){if(!this.canvas||!this.shadowRoot)return;const $=this.shadowRoot.querySelector(".canvas-container");if(!$)return;const J=$.getBoundingClientRect();if(J.width===0||J.height===0)return;const q=window.devicePixelRatio||2;if(this.canvas.width=J.width*q,this.canvas.height=J.height*q,this.canvas.style.width="100%",this.canvas.style.height="100%",this.ctx=this.canvas.getContext("2d"),this.ctx)this.ctx.scale(q,q);this.canvasWidth=J.width,this.canvasHeight=J.height,this.initializeAnimations()}setupCanvas(){if(!this.shadowRoot)return;const $=this.shadowRoot.querySelector(".canvas-container");if(!$)return;const J=$.querySelector("canvas");if(J)J.remove();this.canvas=document.createElement("canvas"),$.appendChild(this.canvas),this.resizeCanvas()}getState($){if(!this.hass||!$)return null;const J=this.hass.states[$];return J?J.state:null}getAttributes($){if(!this.hass||!$)return{};const J=this.hass.states[$];return J?J.attributes:{}}getWeatherData(){const $=this.config.entity||"weather.home",J=this.getState($),q=this.getAttributes($),Q=q.condition||J||"sunny";let K=null;if(this.config.templowAttribute&&q[this.config.templowAttribute]!=null)K=q[this.config.templowAttribute];else{for(let Z of g0)if(q[Z]!=null){K=q[Z];break}if(K==null)K=(q.forecast&&q.forecast[0]?q.forecast[0].templow??null:null)||(q.forecast_hourly&&q.forecast_hourly[0]?q.forecast_hourly[0].native_templow??null:null)}return{condition:Q,temperature:q.temperature!=null?q.temperature:null,apparentTemperature:q.apparent_temperature||null,humidity:q.humidity!=null?q.humidity:null,windSpeed:q.wind_speed!=null?q.wind_speed:null,windGust:q.wind_gust_speed||q.wind_gust||null,windBearing:q.wind_bearing!=null?q.wind_bearing:null,windDirection:q.wind_direction||null,pressure:q.pressure||null,forecast:q.forecast||q.forecast_hourly||[],friendlyName:q.friendly_name||U.t("weather"),templow:K}}getTodayForecast(){if(!this.hass||!this.config)return[];const $=this.getWeatherData();if(!$.forecast||$.forecast.length===0)return[];const J=new Date,q=new Date(J.getFullYear(),J.getMonth(),J.getDate()),Q=new Date(q);Q.setDate(Q.getDate()+1);const K=$.forecast.filter((W)=>{if(!W.datetime)return!1;const V=new Date(W.datetime),B=new Date(V.getFullYear(),V.getMonth(),V.getDate());return B.getTime()===q.getTime()||B.getTime()===Q.getTime()&&V.getHours()<=J.getHours()}),Z=Math.max(1,Math.floor(Number(this.config.hourlyForecastHours??P.hourlyForecastHours)));return K.sort((W,V)=>new Date(W.datetime).getTime()-new Date(V.datetime).getTime()).slice(0,Z)}getWeekForecast(){if(!this.hass||!this.config)return[];const $=this.getWeatherData();if(!$.forecast||$.forecast.length===0)return[];const J=Math.max(1,Math.floor(Number(this.config.dailyForecastDays??P.dailyForecastDays))),q=new Date,Q=new Date(q.getFullYear(),q.getMonth(),q.getDate()),K=new Date(Q);K.setDate(K.getDate()+J);const Z=(V)=>{const B=V.getFullYear(),x=String(V.getMonth()+1).padStart(2,"0"),b=String(V.getDate()).padStart(2,"0");return`${B}-${x}-${b}`},W=new Map;return $.forecast.forEach((V)=>{if(!V.datetime)return;const B=new Date(V.datetime);if(Number.isNaN(B.getTime()))return;if(B<Q||B>=K)return;const x=Z(B),b=Math.abs(B.getHours()+B.getMinutes()/60-12),R=W.get(x);if(!R||b<R.hourScore)W.set(x,{item:V,itemDate:B,hourScore:b})}),Array.from(W.values()).sort((V,B)=>V.itemDate.getTime()-B.itemDate.getTime()).map((V)=>V.item).slice(0,J)}startAnimation(){const $=()=>{this.draw(),this.animationFrame=requestAnimationFrame($)};$()}draw(){if(!this.ctx||!this.canvas)return;if(!this.canvasWidth||!this.canvasHeight){if(this.resizeCanvas(),!this.canvasWidth||!this.canvasHeight)return}const $=this.canvasWidth,J=this.canvasHeight;this.ctx.clearRect(0,0,$,J);const q=this.getWeatherData(),Q=this.hass?.states[this.config.entity],K=V0(Q||{},this.config.sunriseEntity,this.config.sunsetEntity,this.hass),Z=this._testTimeOfDay||B0(K);switch(q.condition.toLowerCase()){case"sunny":case"clear":this.animations.sunny?.draw(Date.now(),$,J,Z);break;case"clear-night":this.animations.sunny?.draw(Date.now(),$,J,{type:"night",progress:0});break;case"rainy":case"rain":this.animations.rainy?.draw(Date.now(),$,J,Z,!1);break;case"pouring":this.animations.rainy?.draw(Date.now(),$,J,Z,!0);break;case"snowy":case"snow":this.animations.snowy?.draw(Date.now(),$,J,Z);break;case"snowy-rainy":this.animations.rainy?.draw(Date.now(),$,J,Z,!1),this.animations.snowy?.draw(Date.now(),$,J,Z);break;case"hail":this.animations.hail?.draw(Date.now(),$,J,Z);break;case"foggy":case"fog":this.animations.foggy?.draw(Date.now(),$,J,Z);break;case"lightning":this.animations.thunderstorm?.draw(Date.now(),$,J,Z,!1);break;case"lightning-rainy":this.animations.thunderstorm?.draw(Date.now(),$,J,Z,!0);break;case"cloudy":case"partlycloudy":default:this.animations.cloudy?.draw(Date.now(),$,J,Z);break}}renderTodayForecast(){const $=this.getTodayForecast();if($.length===0)return M`<div style="opacity: 0.6; font-size: 14px;">${U.t("forecast_unavailable")}</div>`;return M`
    ...
    `}renderDailyForecast(){const $=this.getWeekForecast();if($.length===0)return M`<div style="opacity: 0.6; font-size: 14px;">${U.t("forecast_unavailable")}</div>`;return M`
      <div class="forecast-scroll">
        ${$.map((J)=>M`
          <div class="forecast-item">
            <div class="forecast-time">${q1(J.datetime,U.lang)}</div>
            <div class="forecast-icon">${R0(J.condition||"sunny")}</div>
            <div class="forecast-temp">${Math.round(J.temperature||J.temp||J.native_temperature||0)}°</div>
          </div>
        `)}
      </div>
    `}convertWindSpeed($){if($==null)return null;if(this.config.windSpeedUnit==="kmh")return Math.round($*3.6*10)/10;return $}getWindSpeedUnit(){return this.config.windSpeedUnit==="kmh"?U.t("wind_unit_kmh"):U.t("wind_unit_ms")}formatCurrentTime(){const $=new Date,J=String($.getHours()).padStart(2,"0"),q=String($.getMinutes()).padStart(2,"0");return`${J}:${q}`}startClock(){if(this.clockInterval)clearInterval(this.clockInterval),this.clockInterval=null;if(!this.config.showClock)return;this.currentTime=this.formatCurrentTime(),this.clockInterval=window.setInterval(()=>{this.currentTime=this.formatCurrentTime()},1000)}render(){if(!this.hass)return M`<div>No Home Assistant connection</div>`;const $=this.getWeatherData(),J=this.hass.states[this.config.entity],q=V0(J,this.config.sunriseEntity,this.config.sunsetEntity,this.hass),Q=this._testTimeOfDay||B0(q),K=`weather-card ${Q.type}`,Z=this.config.height?`${this.config.height}px`:"200px",W=t0(Q),V=W?`background: linear-gradient(135deg, rgb(${W.start.r}, ${W.start.g}, ${W.start.b}), rgb(${W.end.r}, ${W.end.g}, ${W.end.b}));`:"",x=`--overlay-opacity: ${this.config.overlayOpacity!==void 0?this.config.overlayOpacity:P.overlayOpacity};`;return M`
      <ha-card
      ...
      >
        <div class="${K}" style="min-height: ${Z}; ${V}; ${x} cursor: pointer;">
          ...
          <div class="content">
            ...
            <div class="details ${this.config.showClock&&this.config.clockPosition==="details"?"details--clock":""}">
              <div class="info-grid">
                ${this.config.showHumidity&&$.humidity!=null?M`
                  <div class="info-item">
                    <span class="info-icon">${u("humidity")}</span>
                    <span>${$.humidity} %</span>
                  </div>
                `:""}
                ${this.config.showSunriseSunset&&q.hasSunData&&q.sunrise?M`
                  <div class="info-item">
                    <span class="info-icon">${u("sunrise")}</span>
                    <span>${W0(q.sunrise)}</span>
                  </div>
                `:""}
                ${this.config.showWind&&$.windSpeed!=null?M`
                  ${this.config.showWindDirection&&$.windBearing!=null?M`
                    <div class="info-item">
                  ...
                  `}
                `:""}
                ${this.config.showSunriseSunset&&q.hasSunData&&q.sunset?M`
                  <div class="info-item">
              ...
              ${this.config.showClock&&this.config.clockPosition==="details"?M`
                <div class="clock">${this.currentTime}</div>
              `:""}
            </div>
            ${this.config.showHourlyForecast?M`
              <div class="forecast-container">
                <div class="forecast-title">${U.t("forecast_title")}</div>
                ${this.renderTodayForecast()}
              </div>
            `:""}
            ${this.config.showDailyForecast?M`
              <div class="forecast-container">
