...
`,D1={sunny:x`
  ...
  `};class N0 extends X{animationFrame=null;canvas=null;ctx=null;canvasWidth=0;canvasHeight=0;animations={};holdTimer=null;holdDelay=500;clockInterval=null;resizeObserver=null;_wheelHandler=null;lastTap=null;holdFired=!1;hourlyForecastSubscription=null;dailyForecastSubscription=null;_testTimeOfDay;static get styles(){return J1}static getConfigElement(){return document.createElement("dynamic-weather-card-editor")}static getStubConfig(){return{type:"custom:dynamic-weather-card",entity:"weather.home",show_hourly_forecast:!0,hourly_forecast_hours:P.hourlyForecastHours,show_daily_forecast:!0,daily_forecast_days:P.dailyForecastDays}}constructor(){super();this.currentTime="";this.hourlyForecast=[];this.dailyForecast=[];this.config={}}connectedCallback(){super.connectedCallback(),this.updateComplete.then(()=>{setTimeout(()=>{if(this.setupCanvas(),this.canvas&&this.ctx)this.initializeAnimations(),this.startAnimation(),this.setupResizeObserver();this.setupForecastScroll(),this.startClock()},100)})}disconnectedCallback(){if(super.disconnectedCallback(),this.animationFrame)cancelAnimationFrame(this.animationFrame),this.animationFrame=null;if(this.resizeObserver)this.resizeObserver.disconnect(),this.resizeObserver=null;if(this._wheelHandler&&this.shadowRoot){const $=this.shadowRoot.querySelector(".forecast-scroll");if($)$.removeEventListener("wheel",this._wheelHandler);this._wheelHandler=null}if(this.clockInterval)clearInterval(this.clockInterval),this.clockInterval=null;this.unsubscribeForecastUpdates()}updated($){if(super.updated($),$.has("hass")||$.has("config")){if(this.canvas&&this.ctx)this.resizeCanvas();if(this.setupForecastScroll(),this.hass&&this.config.entity)this.subscribeForecastUpdates()}if($.has("config"))this.startClock();const J=Z0({configLang:this.config?.language,hassLang:this.hass?.language});if(b.lang!==J)b.setLanguage(J)}initializeAnimations(){if(!this.ctx)return;this.animations={sunny:new M0(this.ctx),rainy:new u(this.ctx),snowy:new P0(this.ctx),cloudy:new b0(this.ctx),foggy:new j0(this.ctx),hail:new x0(this.ctx),thunderstorm:new R0(this.ctx)}}setupResizeObserver(){if(!this.shadowRoot)return;const $=this.shadowRoot.querySelector(".canvas-container");if(!$)return;this.resizeObserver=new ResizeObserver(()=>{this.resizeCanvas()}),this.resizeObserver.observe($)}setupForecastScroll(){if(!this.shadowRoot)return;const $=this.shadowRoot.querySelector(".forecast-scroll");if(!$)return;if(this._wheelHandler)$.removeEventListener("wheel",this._wheelHandler);this._wheelHandler=(J)=>{const q=J;if(q.deltaY!==0)J.preventDefault(),$.scrollLeft+=q.deltaY},$.addEventListener("wheel",this._wheelHandler,{passive:!1})}resizeCanvas(){if(!this.canvas||!this.shadowRoot)return;const $=this.shadowRoot.querySelector(".canvas-container");if(!$)return;const J=$.getBoundingClientRect();if(J.width===0||J.height===0)return;const q=window.devicePixelRatio||2;if(this.canvas.width=J.width*q,this.canvas.height=J.height*q,this.canvas.style.width="100%",this.canvas.style.height="100%",this.ctx=this.canvas.getContext("2d"),this.ctx)this.ctx.scale(q,q);this.canvasWidth=J.width,this.canvasHeight=J.height,this.initializeAnimations()}setupCanvas(){if(!this.shadowRoot)return;const $=this.shadowRoot.querySelector(".canvas-container");if(!$)return;const J=$.querySelector("canvas");if(J)J.remove();this.canvas=document.createElement("canvas"),$.appendChild(this.canvas),this.resizeCanvas()}getState($){if(!this.hass||!$)return null;const J=this.hass.states[$];return J?J.state:null}getAttributes($){if(!this.hass||!$)return{};const J=this.hass.states[$];return J?J.attributes:{}}getWeatherData(){const $=this.config.entity||"weather.home",J=this.getState($),q=this.getAttributes($),Q=q.condition||J||"sunny";let K=null;if(this.config.templowAttribute&&q[this.config.templowAttribute]!=null)K=q[this.config.templowAttribute];else{for(let Z of g0)if(q[Z]!=null){K=q[Z];break}if(K==null)K=(q.forecast&&q.forecast[0]?q.forecast[0].templow??null:null)||(q.forecast_hourly&&q.forecast_hourly[0]?q.forecast_hourly[0].native_templow??null:null)}return{condition:Q,temperature:q.temperature!=null?q.temperature:null,apparentTemperature:q.apparent_temperature||null,humidity:q.humidity!=null?q.humidity:null,windSpeed:q.wind_speed!=null?q.wind_speed:null,windGust:q.wind_gust_speed||q.wind_gust||null,windBearing:q.wind_bearing!=null?q.wind_bearing:null,windDirection:q.wind_direction||null,pressure:q.pressure||null,forecast:q.forecast||q.forecast_hourly||this.hourlyForecast||[],friendlyName:q.friendly_name||b.t("weather"),templow:K}}async subscribeForecastUpdates(){if(!this.hass||!this.config.entity)return;this.unsubscribeForecastUpdates();try{if(this.hourlyForecastSubscription=this.hass.connection.subscribeMessage(($)=>{if($.forecast&&$.forecast.length>0)this.hourlyForecast=$.forecast},{type:"weather/subscribe_forecast",forecast_type:"hourly",entity_id:this.config.entity}),this.config.showDailyForecast)this.dailyForecastSubscription=this.hass.connection.subscribeMessage(($)=>{if($.forecast&&$.forecast.length>0)this.dailyForecast=$.forecast},{type:"weather/subscribe_forecast",forecast_type:"daily",entity_id:this.config.entity})}catch{}}async unsubscribeForecastUpdates(){if(this.hourlyForecastSubscription){try{(await this.hourlyForecastSubscription)()}catch{}this.hourlyForecastSubscription=null}if(this.dailyForecastSubscription){try{(await this.dailyForecastSubscription)()}catch{}this.dailyForecastSubscription=null}}getTodayForecast(){if(!this.hass||!this.config)return[];const $=Math.max(1,Math.floor(Number(this.config.hourlyForecastHours??P.hourlyForecastHours)));if(this.hourlyForecast&&this.hourlyForecast.length>0)return this.hourlyForecast.slice(0,$);const J=this.getWeatherData();if(!J.forecast||J.forecast.length===0)return[];const q=new Date,Q=new Date(q.getFullYear(),q.getMonth(),q.getDate()),K=new Date(Q);return K.setDate(K.getDate()+1),J.forecast.filter((V)=>{if(!V.datetime)return!1;const W=new Date(V.datetime),B=new Date(W.getFullYear(),W.getMonth(),W.getDate());return B.getTime()===Q.getTime()||B.getTime()===K.getTime()&&W.getHours()<=q.getHours()}).sort((V,W)=>new Date(V.datetime).getTime()-new Date(W.datetime).getTime()).slice(0,$)}getWeekForecast(){if(!this.hass||!this.config)return[];if(this.dailyForecast&&this.dailyForecast.length>0){const W=Math.max(1,Math.floor(Number(this.config.dailyForecastDays??P.dailyForecastDays)));return this.dailyForecast.slice(0,W)}const $=this.getWeatherData();if(!$.forecast||$.forecast.length===0)return[];const J=Math.max(1,Math.floor(Number(this.config.dailyForecastDays??P.dailyForecastDays))),q=new Date,Q=new Date(q.getFullYear(),q.getMonth(),q.getDate()),K=new Date(Q);K.setDate(K.getDate()+J);const Z=(W)=>{const B=W.getFullYear(),R=String(W.getMonth()+1).padStart(2,"0"),M=String(W.getDate()).padStart(2,"0");return`${B}-${R}-${M}`},V=new Map;return $.forecast.forEach((W)=>{if(!W.datetime)return;const B=new Date(W.datetime);if(Number.isNaN(B.getTime()))return;if(B<Q||B>=K)return;const R=Z(B),M=Math.abs(B.getHours()+B.getMinutes()/60-12),_=V.get(R);if(!_||M<_.hourScore)V.set(R,{item:W,itemDate:B,hourScore:M})}),Array.from(V.values()).sort((W,B)=>W.itemDate.getTime()-B.itemDate.getTime()).map((W)=>W.item).slice(0,J)}startAnimation(){const $=()=>{this.draw(),this.animationFrame=requestAnimationFrame($)};$()}draw(){if(!this.ctx||!this.canvas)return;if(!this.canvasWidth||!this.canvasHeight){if(this.resizeCanvas(),!this.canvasWidth||!this.canvasHeight)return}const $=this.canvasWidth,J=this.canvasHeight;this.ctx.clearRect(0,0,$,J);const q=this.getWeatherData(),Q=this.hass?.states[this.config.entity],K=V0(Q||{},this.config.sunriseEntity,this.config.sunsetEntity,this.hass),Z=this._testTimeOfDay||B0(K);switch(q.condition.toLowerCase()){case"sunny":case"clear":this.animations.sunny?.draw(Date.now(),$,J,Z);break;case"clear-night":this.animations.sunny?.draw(Date.now(),$,J,{type:"night",progress:0});break;case"rainy":case"rain":this.animations.rainy?.draw(Date.now(),$,J,Z,!1);break;case"pouring":this.animations.rainy?.draw(Date.now(),$,J,Z,!0);break;case"snowy":case"snow":this.animations.snowy?.draw(Date.now(),$,J,Z);break;case"snowy-rainy":this.animations.rainy?.draw(Date.now(),$,J,Z,!1),this.animations.snowy?.draw(Date.now(),$,J,Z);break;case"hail":this.animations.hail?.draw(Date.now(),$,J,Z);break;case"foggy":case"fog":this.animations.foggy?.draw(Date.now(),$,J,Z);break;case"lightning":this.animations.thunderstorm?.draw(Date.now(),$,J,Z,!1);break;case"lightning-rainy":this.animations.thunderstorm?.draw(Date.now(),$,J,Z,!0);break;case"cloudy":case"partlycloudy":default:this.animations.cloudy?.draw(Date.now(),$,J,Z);break}}renderTodayForecast(){const $=this.getTodayForecast();if($.length===0)return j`<div style="opacity: 0.6; font-size: 14px;">${b.t("forecast_unavailable")}</div>`;return j`
    ...
    `}renderDailyForecast(){const $=this.getWeekForecast();if($.length===0)return j`<div style="opacity: 0.6; font-size: 14px;">${b.t("forecast_unavailable")}</div>`;return j`
      <div class="forecast-scroll">
        ${$.map((J)=>j`
          <div class="forecast-item">
    ...
    `}convertWindSpeed($){if($==null)return null;const J=this.config.entity||"weather.home";if(this.getAttributes(J).wind_speed_unit)return Math.round($*10)/10;if(this.config.windSpeedUnit==="kmh")return Math.round($*3.6*10)/10;return Math.round($*10)/10}getWindSpeedUnit(){const $=this.config.entity||"weather.home",q=this.getAttributes($).wind_speed_unit;if(q){const Q=q.toLowerCase().replace(/[^a-z]/g,"");if(Q==="kmh"||Q==="kmph")return b.t("wind_unit_kmh");else if(Q==="ms"||Q==="mps")return b.t("wind_unit_ms");else if(Q==="mph")return b.t("wind_unit_mph");else if(Q==="knots"||Q==="kn"||Q==="kt")return b.t("wind_unit_knots");else if(Q==="fts"||Q==="ftps")return b.t("wind_unit_fts");return q}return this.config.windSpeedUnit==="kmh"?b.t("wind_unit_kmh"):b.t("wind_unit_ms")}formatCurrentTime(){const $=new Date,J=String($.getHours()).padStart(2,"0"),q=String($.getMinutes()).padStart(2,"0");return`${J}:${q}`}startClock(){if(this.clockInterval)clearInterval(this.clockInterval),this.clockInterval=null;if(!this.config.showClock)return;this.currentTime=this.formatCurrentTime(),this.clockInterval=window.setInterval(()=>{this.currentTime=this.formatCurrentTime()},1000)}render(){if(!this.hass)return j`<div>No Home Assistant connection</div>`;const $=this.getWeatherData(),J=this.hass.states[this.config.entity],q=V0(J,this.config.sunriseEntity,this.config.sunsetEntity,this.hass),Q=this._testTimeOfDay||B0(q),K=`weather-card ${Q.type}`,Z=this.config.height?`${this.config.height}px`:"200px",V=t0(Q),W=V?`background: linear-gradient(135deg, rgb(${V.start.r}, ${V.start.g}, ${V.start.b}), rgb(${V.end.r}, ${V.end.g}, ${V.end.b}));`:"",R=`--overlay-opacity: ${this.config.overlayOpacity!==void 0?this.config.overlayOpacity:P.overlayOpacity};`;return j`
      ...
      >
        <div class="${K}" style="min-height: ${Z}; ${W}; ${R} cursor: pointer;">
          <div class="canvas-container"></div>
          <div class="content">
            ${this.config.name!==void 0?j`
              <div class="header">
            ...
            <div class="details ${this.config.showClock&&this.config.clockPosition==="details"?"details--clock":""}">
              <div class="info-grid">
                ${this.config.showHumidity&&$.humidity!=null?j`
                  <div class="info-item">
                    <span class="info-icon">${m("humidity")}</span>
                    <span>${$.humidity} %</span>
                  </div>
                `:""}
                ${this.config.showSunriseSunset&&q.hasSunData&&q.sunrise?j`
                  <div class="info-item">
                    <span class="info-icon">${m("sunrise")}</span>
                    <span>${W0(q.sunrise)}</span>
                  </div>
                `:""}
                ${this.config.showWind&&$.windSpeed!=null?j`
                  ${this.config.showWindDirection&&$.windBearing!=null?j`
                    <div class="info-item">
                  ...
                  `}
                `:""}
                ${this.config.showSunriseSunset&&q.hasSunData&&q.sunset?j`
                  <div class="info-item">
                    <span class="info-icon">${m("sunset")}</span>
                    <span>${W0(q.sunset)}</span>
                  </div>
                `:""}
              </div>
              ${this.config.showClock&&this.config.clockPosition==="details"?j`
                <div class="clock">${this.currentTime}</div>
              `:""}
            </div>
            ${this.config.showHourlyForecast?j`
              <div class="forecast-container">
                <div class="forecast-title">${b.t("forecast_title")}</div>
                ${this.renderTodayForecast()}
              </div>
            `:""}
            ${this.config.showDailyForecast?j`
              <div class="forecast-container">
    ...
    `}setConfig($){if(!$.entity)throw new Error("Please define a weather entity");const J=$.show_hourly_forecast??$.show_forecast;if(this.config={type:"custom:dynamic-weather-card",entity:$.entity,icons_path:$.icons_path,name:$.name,height:$.height||P.height,showFeelsLike:$.show_feels_like!==!1,showWind:$.show_wind!==!1,showWindGust:$.show_wind_gust!==!1,showWindDirection:$.show_wind_direction!==!1,showHumidity:$.show_humidity!==!1,showMinTemp:$.show_min_temp!==!1,showForecast:$.show_forecast===!0,showHourlyForecast:J===!0,showDailyForecast:$.show_daily_forecast===!0,hourlyForecastHours:$.hourly_forecast_hours??P.hourlyForecastHours,dailyForecastDays:$.daily_forecast_days??P.dailyForecastDays,showSunriseSunset:$.show_sunrise_sunset!==!1,showClock:$.show_clock===!0,clockPosition:$.clock_position||P.clockPosition,overlayOpacity:$.overlay_opacity!==void 0?$.overlay_opacity:P.overlayOpacity,language:$.language||P.language,windSpeedUnit:$.wind_speed_unit||P.windSpeedUnit,sunriseEntity:$.sunrise_entity||null,sunsetEntity:$.sunset_entity||null,templowAttribute:$.templow_attribute||null,tapAction:$.tap_action||{action:"more-info"},holdAction:$.hold_action||{action:"none"},doubleTapAction:$.double_tap_action||{action:"none"}},this.config.language)b.setLanguage(this.config.language)}handleAction($){if(!$||!this.hass)return;switch($.action||"more-info"){case"more-info":this.fireEvent("hass-more-info",{entityId:$.entity||this.config.entity});break;case"toggle":this.hass.callService("homeassistant","toggle",{entity_id:$.entity||this.config.entity});break;case"call-service":if($.service){const[q,Q]=$.service.split(".");this.hass.callService(q,Q,$.service_data||{})}break;case"navigate":if($.navigation_path)window.history.pushState(null,"",$.navigation_path),this.fireEvent("location-changed",{replace:!1});break;case"url":if($.url_path)window.open($.url_path);break;case"none":default:break}}fireEvent($,J={}){const q=new CustomEvent($,{detail:J,bubbles:!0,composed:!0});this.dispatchEvent(q)}handleTap($){if($.target.closest(".forecast-item")||$.target.closest(".info-item"))return;if(this.lastTap&&Date.now()-this.lastTap<300){this.handleDoubleTap($),this.lastTap=null;return}this.lastTap=Date.now(),setTimeout(()=>{if(this.lastTap)this.handleAction(this.config.tapAction),this.lastTap=null},300)}handlePointerDown($){this.holdTimer=window.setTimeout(()=>{this.handleHold($),this.holdFired=!0},this.holdDelay)}handlePointerUp($){if(this.holdTimer)clearTimeout(this.holdTimer);if(this.holdFired)$.preventDefault(),$.stopPropagation(),this.holdFired=!1}handleHold($){this.handleAction(this.config.holdAction)}handleDoubleTap($){this.handleAction(this.config.doubleTapAction)}getCardSize(){return 1}}G([I({type:Object})],N0.prototype,"hass",void 0),G([I({type:Object})],N0.prototype,"config",void 0),G([E()],N0.prototype,"currentTime",void 0),G([E()],N0.prototype,"hourlyForecast",void 0),G([E()],N0.prototype,"dailyForecast",void 0);class U0 extends X{constructor(){super(...arguments);this._config={}}setConfig($){this._config={name:"",height:P.height,show_feels_like:P.showFeelsLike,show_wind:P.showWind,show_wind_gust:P.showWindGust,show_wind_direction:P.showWindDirection,show_humidity:P.showHumidity,show_min_temp:P.showMinTemp,show_hourly_forecast:P.showHourlyForecast,hourly_forecast_hours:P.hourlyForecastHours,show_daily_forecast:P.showDailyForecast,daily_forecast_days:P.dailyForecastDays,show_sunrise_sunset:P.showSunriseSunset,show_clock:P.showClock,clock_position:P.clockPosition,overlay_opacity:P.overlayOpacity,language:P.language,wind_speed_unit:P.windSpeedUnit,sunrise_entity:"",sunset_entity:"",...$}}get _schema(){return[{name:"entity",required:!0,selector:{entity:{domain:["weather"]}}},{name:"name",selector:{text:{}}},{name:"height",selector:{number:{min:200,max:800,step:10,mode:"box"}}},{name:"show_feels_like",selector:{boolean:{}}},{name:"show_wind",selector:{boolean:{}}},{name:"show_wind_gust",selector:{boolean:{}}},{name:"show_wind_direction",selector:{boolean:{}}},{name:"show_humidity",selector:{boolean:{}}},{name:"show_min_temp",selector:{boolean:{}}},{name:"show_hourly_forecast",selector:{boolean:{}}},{name:"hourly_forecast_hours",selector:{number:{min:1,max:24,step:1,mode:"box"}}},{name:"show_daily_forecast",selector:{boolean:{}}},{name:"daily_forecast_days",selector:{number:{min:1,max:14,step:1,mode:"box"}}},{name:"show_sunrise_sunset",selector:{boolean:{}}},{name:"sunrise_entity",selector:{entity:{domain:["sensor"]}}},{name:"sunset_entity",selector:{entity:{domain:["sensor"]}}},{name:"show_clock",selector:{boolean:{}}},{name:"clock_position",selector:{select:{options:[{label:"Top",value:"top"},{label:"Details",value:"details"}]}}},{name:"overlay_opacity",selector:{number:{min:0,max:1,step:0.05,mode:"box"}}},{name:"language",selector:{select:{options:[{label:"Auto",value:"auto"},{label:"English",value:"en"},{label:"Russian",value:"ru"},{label:"German",value:"de"},{label:"Dutch",value:"nl"},{label:"French",value:"fr"},{label:"Spanish",value:"es"}]}}},{name:"wind_speed_unit",selector:{select:{options:[{label:"m/s",value:"ms"},{label:"km/h",value:"kmh"}]}}}]}_computeLabel=($)=>{return{entity:"Weather Entity",name:"Card Title",height:"Card Height",show_feels_like:"Show Feels Like",show_wind:"Show Wind Speed",show_wind_gust:"Show Wind Gust",show_wind_direction:"Show Wind Direction",show_humidity:"Show Humidity",show_min_temp:"Show Min Temperature",show_hourly_forecast:"Show Hourly Forecast",hourly_forecast_hours:"Hourly Forecast Hours",show_daily_forecast:"Show Daily Forecast",daily_forecast_days:"Daily Forecast Days",show_sunrise_sunset:"Show Sunrise/Sunset",show_clock:"Show Clock",clock_position:"Clock Position",overlay_opacity:"Overlay Opacity",language:"Language",wind_speed_unit:"Wind Speed Unit",sunrise_entity:"Sunrise Entity",sunset_entity:"Sunset Entity"}[$.name]??$.name};_valueChanged($){const J=$.detail?.value;if(!J)return;this._config=J,this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:this._config},bubbles:!0,composed:!0}))}render(){if(!this.hass)return j``;return j`
      ...
      ></ha-form>
    `}}G([I({attribute:!1})],U0.prototype,"hass",void 0),G([E()],U0.prototype,"_config",void 0);var K1={ATTRIBUTE:1,CHILD:2,PROPERTY:3,BOOLEAN_ATTRIBUTE:4,EVENT:5,ELEMENT:6},k0=($)=>(...J)=>({_$litDirective$:$,values:J});class H0{constructor($){}get _$AU(){return this._$AM._$AU}_$AT($,J,q){this._$Ct=$,this._$AM=J,this._$Ci=q}_$AS($,J){return this.update($,J)}update($,J){return this.render(...J)}}var Z1=($)=>$.strings===void 0;var w1=function($){this._$AN!==void 0?(s(this),this._$AM=$,W1(this)):this._$AM=$},f1=function($,J=!1,q=0){const Q=this._$AH,K=this._$AN;if(K!==void 0&&K.size!==0)if(J)if(Array.isArray(Q))for(let Z=q;Z<Q.length;Z++)c(Q[Z],!1),s(Q[Z]);else Q!=null&&(c(Q,!1),s(Q));else c(this,$)},c=($,J)=>{const q=$._$AN;if(q===void 0)return!1;for(let Q of q)Q._$AO?.(J,!1),c(Q,J);return!0},s=($)=>{let J,q;do{if((J=$._$AM)===void 0)break;q=J._$AN,q.delete($),$=J}while(q?.size===0)},W1=($)=>{for(let J;J=$._$AM;$=J){let q=J._$AN;if(q===void 0)J._$AN=q=new Set;else if(q.has($))break;q.add($),y1(J)}},y1=($)=>{$.type==K1.CHILD&&($._$AP??=f1,$._$AQ??=w1)};class L0 extends H0{constructor(){super(...arguments),this._$AN=void 0}_$AT($,J,q){super._$AT($,J,q),W1(this),this.isConnected=$._$AU}_$AO($,J=!0){$!==this.isConnected&&(this.isConnected=$,$?this.reconnected?.():this.disconnected?.()),J&&(c(this,$),s(this))}setValue($){if(Z1(this._$Ct))this._$Ct._$AI($,this);else{const J=[...this._$Ct._$AH];J[this._$Ci]=$,this._$Ct._$AI(J,this,0)}}disconnected(){}reconnected(){}}class V1 extends L0{constructor(){super(...arguments)}_key="";_onLangChange=null;render($){return this._key=$,b.t($)}reconnected(){this._onLangChange=()=>{this.setValue(b.t(this._key))},window.addEventListener("language-changed",this._onLangChange)}disconnected(){if(this._onLangChange)window.removeEventListener("language-changed",this._onLangChange)}}var l1=k0(V1);try{customElements.define("dynamic-weather-card",N0),customElements.define("dynamic-weather-card-editor",U0),console.log(`%cDynamic Weather Card %c${c0}`,"color: #007AFF; font-weight: bold; font-size: 14px;","color: #666; font-size: 12px;",`
\u0414\u0438\u043D\u0430\u043C\u0438\u0447\u0435\u0441\u043A\u0430\u044F \u043A\u0430\u0440\u0442\u043E\u0447\u043A\u0430 \u043F\u043E\u0433\u043E\u0434\u044B`),window.customCards=window.customCards||[];const $={type:"dynamic-weather-card",name:"Dynamic Weather Card",description:"\u0414\u0438\u043D\u0430\u043C\u0438\u0447\u0435\u0441\u043A\u0430\u044F \u043A\u0430\u0440\u0442\u043E\u0447\u043A\u0430 \u043F\u043E\u0433\u043E\u0434\u044B",preview:!0,documentationURL:"https://github.com/teuchezh/dynamic-weather-card"};window.customCards.push($)}catch($){console.error("\u274C \u041E\u0448\u0438\u0431\u043A\u0430 \u043F\u0440\u0438 \u0440\u0435\u0433\u0438\u0441\u0442\u0440\u0430\u0446\u0438\u0438 Dynamic Weather Card:",$)}export{l1 as t,Z0 as resolveLanguage,b as i18n};
